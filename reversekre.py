...
ey_train = toaa(ey_train)
ey_test = toaa(ey_test)
...
print('Train 2...')
model3.fit(eX_train[:1000], ey_train[:1000],
          batch_size=batch_size,
          nb_epoch=4,show_accuracy=True)
'''
acc = result(y_test,
               findmax(np.array(model2.predict(X_test,
                                               batch_size=batch_size))))
...
aacc = result(ey_test,
               efindmax(np.array(model3.predict(eX_test,
                                               batch_size=batch_size))))
