...
(X1, Y1) =cPickle.load(open('140-test-all.pkl', 'rb'))
...
(X2, Y2) =cPickle.load(open('emoji.pkl', 'rb'))


eeX_train, eeX_test, eey_train, eey_test = train_test_split(X2, Y2, test_size=0.2, random_state=42)
...
model.add_node(LSTM(100), name='forward', input='embedding')
model.add_node(LSTM(100, go_backwards=True), name='backward', input='embedding')
...
#model2.add_input(name='input1', input_shape=(maxlen,), dtype=int)
model4.add(model)
model4.add(Dense(70, activation='sigmoid'))
#(maxlen,model2.add_output(name='output1', input='sigmoid')
...
for i in np.arange(0,1,0.1):
  ...
  model3.fit(eX_train[np.int(l2*i):np.int(l2*(i+0.1))], ey_train[np.int(l2*i):np.int(l2*(i+0.1))],
          batch_size=batch_size,
          nb_epoch=1,show_accuracy=True)
  model4.fit(eeX_train[np.int(l3*i):np.int(l3*(i+0.1))], eey_train[np.int(l3*i):np.int(l3*(i+0.1))],
          batch_size=batch_size,
          nb_epoch=1,show_accuracy=True)
'''
print('Train...')
...
aacc = result(ey_test,findmax(model3.predict(eX_test,batch_size=batch_size)))

#y_pred=efindmax(np.array(model3.predict(eX_test,batch_size=batch_size)))

print('Test accuracy:', aacc)

aaccaa = result(eey_test,efindmax(model4.predict(eeX_test,batch_size=batch_size)))
